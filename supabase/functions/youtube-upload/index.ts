...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "callback") {
      ...
      return new Response(
        `<html><body style="font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;background:#0f1729;color:#f0f0f0;">
          <div style="text-align:center;">
            <h2 style="color:#d4a843;">✅ YouTube conectado!</h2>
            <p>Canal: ${channelTitle || "Conectado"}</p>
            <p style="color:#888;">Esta janela fechará automaticamente...</p>
          </div>
          <script>
            if(window.opener){window.opener.postMessage({type:'youtube_connected',channel:'${channelTitle || ""}'},'*');}
